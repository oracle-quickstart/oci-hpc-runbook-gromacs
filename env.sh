...
export TF_VAR_image=$(oci --region ${TF_VAR_region} compute image list --compartment-id ${TF_VAR_targetCompartment} --all | jq -r '.data[] | select(."display-name"==env.TF_VAR_image_name).id')export TF_VAR_instance_pool_shape="BM.GPU3.8"
